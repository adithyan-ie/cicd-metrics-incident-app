...
The main app README lists the available observability endpoints.
...
- [x] Apache Kafka producer for Jenkins/GitHub Actions events in `../cicd-pipeline-app`
- [x] Apache Flink stream processor for real-time metric updates in `../cicd-pipeline-app`
...
### Jenkins
See `../cicd-pipeline-app/Jenkinsfile` for Jenkins pipeline integration.

### GitHub Actions
Use `POST /dora/api/events` or `POST /api/observability/events` from a workflow step.

### GitLab CI
Use `POST /dora/api/events` or `POST /api/observability/events` from a pipeline job.
...
**Questions?** Check `README.md` for endpoint documentation and integration notes.
